...
- Method: Individual 3D-separable WMH regions are labeled, then classified based on overlap with dilated ventricle mask
...
*Figure: Examples of all three WMH classification approaches applied to the same scan. The continuous method uses morphological dilation, while the 10mm and juxta methods use Euclidean distance transforms.*

### Technical Implementation

**Preprocessing:**
- Ventricle masks are automatically dilated by the minimum voxel resolution to compensate for potential partial volume effects in ventricular segmentation

**Continuous Method:**
1. Dilate WMH mask to connect nearby lesions
2. Label individual 3D-separable WMH regions using connected component analysis
3. Dilate ventricle mask to define periventricular zone
4. Classify each WMH region: overlapping with dilated ventricles → pWMH, otherwise → sWMH
5. Constrain to original WMH boundaries
...
### Install
```bash
git clone https://github.com/yourusername/wmh-classifier.git
cd wmh-classifier
...
- `--distance-thresholds T1,T2,...` - Distance thresholds in mm (comma-separated)
  - If not provided, uses continuous (dilation-based) method
  - Examples: `10` for binary, `3,13` for juxta method
- `--zone-names NAME1,NAME2,...` - Custom zone names (must match number of zones)

**Continuous Method Parameters (only used when no distance thresholds):**
- `--wmh-dilation N` - WMH dilation iterations (default: 1)
- `--vent-dilation N` - Ventricle dilation iterations (default: 1)
...
- `total_volume_ml` - Total WMH volume in milliliters

**Zone-Specific Columns (dynamic based on method):**
- `zone1_voxels`, `zone1_volume_ml` - Zone 1 statistics
- `zone2_voxels`, `zone2_volume_ml` - Zone 2 statistics
- `zone3_voxels`, `zone3_volume_ml` - Zone 3 statistics (if applicable)
...
### Example CSV Output
```csv
wmh_file,ventricle_file,method,zone_mapping,total_voxels,total_volume_ml,zone1_voxels,zone1_volume_ml,zone2_voxels,zone2_volume_ml,zone3_voxels,zone3_volume_ml,classified_mask
/data/sub-001_WMH.nii.gz,/data/sub-001_vent.nii.gz,continuous,"1:periventricular;2:subcortical",15234,12.4567,11234,9.1872,4000,3.2695,,,sub-001_WMH_wmhc-cont_01.nii.gz
/data/sub-001_WMH.nii.gz,/data/sub-001_vent.nii.gz,0-10,"1:0-10mm;2:>10mm",15234,12.4567,9500,7.7634,5734,4.6933,,,sub-001_WMH_wmhc-0-10_01.nii.gz
/data/sub-001_WMH.nii.gz,/data/sub-001_vent.nii.gz,0-3-13,"1:juxta;2:peri;3:deep",15234,12.4567,3200,2.6145,8000,6.5389,4034,3.3033,sub-001_WMH_wmhc-0-3-13_01.nii.gz
```
...
**CSV Output:**
```csv
wmh_file,ventricle_file,method,zone_mapping,total_voxels,total_volume_ml,zone1_voxels,zone1_volume_ml,zone2_voxels,zone2_volume_ml,classified_mask
data/sub-001/sub-001_WMH.nii.gz,data/sub-001/sub-001_ventricles.nii.gz,continuous,"1:periventricular;2:subcortical",15234,12.4567,11234,9.1872,4000,3.2695,results/sub-001_WMH_wmhc-cont_01.nii.gz
```
...
# Output:
#   results/sub-001_10mm.csv
#   results/sub-001_WMH_wmhc-0-10_01.nii.gz
```
...
# Output:
#   results/sub-001_juxta.csv
#   results/sub-001_WMH_wmhc-0-3-13_01.nii.gz (with labels 1=juxta, 2=peri, 3=deep)
```

### Example 4: Custom Multi-Zone Classification
```bash
# Create 4 zones: 0-5mm, 5-10mm, 10-20mm, >20mm
python wmh_classifier.py \
    -i data/sub-001/sub-001_WMH.nii.gz \
    -v data/sub-001/sub-001_ventricles.nii.gz \
    -o results/sub-001_custom.csv \
    --distance-thresholds 5,10,20 \
    --zone-names immediate,near,moderate,far \
    --save-masks

# Output:
#   results/sub-001_custom.csv
#   results/sub-001_WMH_wmhc-0-5-10-20_01.nii.gz
```

### Example 5: Batch Processing with Automated CSV Generation
```bash
...
# Output:
#   batch_results_juxta.csv (N rows, one per subject)
#   {basename}_wmhc-0-3-13_01.nii.gz (for each subject)
```
...
|--------|-------|------------|----------|
| **Continuous** | 2 | Adjacent vs non-adjacent to ventricles | Traditional binary classification, maintains individual lesion integrity |
...
- When voxel resolution is coarse relative to ventricle size
...
- **Type:** Binary or integer mask (non-zero voxels = WMH)
- **Space:** Must be co-registered with ventricle mask

### Ventricle Mask
- **Format:** NIfTI (`.nii` or `.nii.gz`)
- **Type:** Binary or integer mask (non-zero voxels = ventricles)
- **Space:** Must be co-registered with WMH mask
- **Note:** Typically represents lateral ventricles only

### Critical Requirements
- Both masks must have identical:
  - Dimensions (matrix size)
  - Voxel spacing
  - Orientation
  - Affine transformation
- Masks should represent the same anatomical space (co-registered)
- Voxel spacing is automatically extracted from NIfTI headers
...
2. Dilate WMH mask by N iterations (default 1)
   → Connects nearby lesions
3. Label connected components in dilated WMH
   → Identifies individual WMH regions
4. Dilate ventricle mask by M iterations (default 1)
   → Defines periventricular zone
   → Compensates for partial volume effects
5. For each WMH region:
   IF overlaps with dilated ventricles:
      Label as periventricular (1)
   ELSE:
      Label as subcortical (2)
6. Multiply by original WMH to constrain boundaries
```

**Parameters:**
- `--wmh-dilation`: Controls lesion connectivity (increase for fragmented WMH)
- `--vent-dilation`: Controls periventricular zone size (increase for broader definition)

### Distance-Based Methods
```
1. Load and binarize inputs
2. Extract voxel spacing (sx, sy, sz) from NIfTI header
...
**Advantages:**
- Exact distances (not affected by dilation parameters)
...
print(f"Total WMH volume: {result['total_volume_ml']:.2f} mL")
print(f"Zone 1: {result['zone1_voxels']} voxels ({result['zone1_volume_ml']:.2f} mL)")
print(f"Zone 2: {result['zone2_voxels']} voxels ({result['zone2_volume_ml']:.2f} mL)")

# Save classified mask
from wmh_classifier import save_classified_mask
mask_path = save_classified_mask(
    result['output_mask'],
    result['affine'],
    result['header'],
    'data/sub-001_WMH.nii.gz',
    'output/stats.csv',
    result['method']
)
```
...
1. **Visual inspection:** Overlay classified masks on original images
2. **Check empty zones:** Verify expected zones have WMH
3. **Compare methods:** Run multiple methods on same data to ensure consistency
4. **Validate co-registration:** Ensure WMH and ventricle masks align properly
5. **Check voxel spacing:** Verify distance calculations make sense for your resolution

### Common Issues

**High periventricular percentage (>90%):**
- Increase `--vent-dilation` less (or decrease for distance methods)
- Check if ventricle mask is too large
- Verify co-registration quality

**All WMH classified as subcortical:**
- Check ventricle mask is not empty
- Verify masks are in same space
- For continuous: increase `--vent-dilation`

**Unexpected zone boundaries in distance methods:**
- Verify voxel spacing is correct in NIfTI headers
- Check for anisotropic voxels (different x,y,z dimensions)

## Performance Considerations

**Continuous method:**
- Faster (no distance transform)
- Memory efficient
- Scales linearly with number of WMH regions
...
**Typical processing times** (on standard workstation):
- Continuous: 5-10 seconds per subject
- Distance-based: 10-20 seconds per subject
...
```bibtex
@software{wmh_classifier,
  author = {Your Name},
  title = {WMH Classifier: Spatial Stratification of White Matter Hyperintensities},
  year = {2025},
  url = {https://github.com/yourusername/wmh-classifier}
}
```

**Related Publications:**
- Include your paper citation here once published
- References for the three classification methods
...
- **Issues:** Open an issue on GitHub with:
  - Error messages
  - Input file characteristics (dimensions, spacing)
  - Command used
